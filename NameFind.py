...
def ID2Name(ID, conf):
    if ID > 0:
        ...
        NameString = " Face Not Recognized "  # Find the Name using the index of the ID
...
def DispID(x, y, w, h, NAME, Image):
 ...
 #  ------------------------------------    THE DRAWING OF THE BOX AND ID   --------------------------------------
    ...
    cv2.putText(Image, NAME, (int(Name_X_pos), int(Name_y_pos - 10)), font, 0.4, RED, stroke, cv2.LINE_AA)                         # Print the name of the ID
